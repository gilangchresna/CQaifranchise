...
serve(async (req) => {
  ...
  try {
    ...
    const { data: sales30 } = await sb.from("sales_transactions")
    ...
    (sales30 || []).forEach(s => {
      ...
      const amt = Number(s.amount || 0) * sgd(cmap[s.outlet_id] || "SGD");
      if (!dailyTotals[s.outlet_id]) dailyTotals[s.outlet_id] = {};
      dailyTotals[s.outlet_id][day] = (dailyTotals[s.outlet_id][day] || 0) + amt;
    });

    const { data: todayTxns } = await sb.from("sales_transactions")
      .select("outlet_id, amount");

    const todayMap = {};
    (todayTxns || []).forEach(t => {
      todayMap[t.outlet_id] = (todayMap[t.outlet_id] || 0) + Number(t.amount || 0) * sgd(cmap[t.outlet_id] || "SGD");
    });
    ...
    for (const [oid, daily] of Object.entries(dailyTotals)) {
      ...
      sb.from("ml_anomaly_scores").upsert(rec).catch(() => {});
    }
  ...
  try {
    ...
    const { data: recent7 } = await sb.from("sales_transactions")
  ...
  try {
    const { data: crits } = await sb.from("ml_anomaly_scores")
    ...
    for (const c of (crits || [])) {
      const { data: dup } = await sb.from("alerts")
        .select("id")
        .eq("outlet_id", c.outlet_id)
        .eq("status", "NEW")
        .eq("type", "SALES_ANOMALY");
      if ((dup || []).length > 0) continue;
      const o = omap[c.outlet_id];
      await sb.from("alerts").insert({
        outlet_id: c.outlet_id,
        type: "SALES_ANOMALY",
        severity: "P0_CRITICAL",
        status: "NEW",
        title: `Sales Anomaly CRITICAL at ${o ? o.code : c.outlet_id}`,
        description: `Anomaly z=${c.anomaly_score?.toFixed(2)}. Review outlet immediately.`,
      }).catch(() => {});
      created++;
    }
    out.alerts = created;
