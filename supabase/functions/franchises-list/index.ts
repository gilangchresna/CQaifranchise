...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Use service role for admin operations, anon for data queries (to respect RLS)
    ...
    if (authHeader.startsWith("Bearer ")) {
      ...
      try {
        ...
        if (userRes.ok) {
          ...
          // Get REAL role from user_profiles, not from UI param
          ...
          if (profile?.role) {
            userRole = profile.role; // Use REAL role from DB
          } else {
            // Fallback to UI param if profile not found
            const roleMap: Record<string, string> = {
              "HQ": "HQ_ADMIN",
              "Regional": "REGIONAL_MANAGER",
              "Franchisee": "FRANCHISEE_OWNER",
            };
            userRole = roleParam ? (roleMap[roleParam] || roleParam) : "HQ_ADMIN";
          }
