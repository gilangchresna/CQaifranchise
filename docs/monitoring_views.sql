-- =============================================================================
-- CyberQuote MVP - Observability Views (Revised)
-- =============================================================================
...
FROM public.cases c
LEFT JOIN public.user_profiles up ON up.id = c.assigned_to_id
...
-- =============================================================================
-- VIEW: ML Model Performance (with graceful fallback if ml_models exists)
-- =============================================================================
CREATE OR REPLACE VIEW public.v_ml_performance_metrics AS
SELECT 
    'anomaly' AS model_type,
...
GROUP BY 'anomaly', CURRENT_DATE
UNION ALL
SELECT 
    'stockout' AS model_type,
    CURRENT_DATE AS score_date,
    COUNT(*) AS total_scores,
    AVG(score) AS avg_score,
    MIN(score) AS min_score,
    ...
    AVG(data_points) AS avg_data_points
FROM public.ml_models
WHERE model_type = 'stockout'
  AND updated_at >= NOW() - INTERVAL '30 days'
GROUP BY 'stockout', CURRENT_DATE;
...
SELECT 
    model_type,
    COUNT(*) AS total_scores,
    COUNT(*) FILTER (WHERE updated_at >= NOW() - INTERVAL '1 hour') AS scores_last_hour,
    COUNT(*) FILTER (WHERE updated_at >= CURRENT_DATE) AS scores_today,
    COUNT(*) FILTER (WHERE is_anomaly = true AND updated_at >= CURRENT_DATE) AS anomalies_today,
    AVG(score) FILTER (WHERE updated_at >= CURRENT_DATE) AS avg_score_today,
    MAX(updated_at) AS last_score_at,
    EXTRACT(EPOCH FROM (NOW() - MAX(updated_at))) / 60 AS minutes_since_last_score
FROM public.ml_models
GROUP BY model_type;
...
LEFT JOIN LATERAL (
    SELECT 
        COUNT(*) FILTER (WHERE c.status NOT IN ('CLOSED', 'RESOLVED')) AS open_cases,
        COUNT(*) FILTER (
    ...
    FROM public.cases c
    JOIN public.alerts a2 ON a2.id = c.alert_id
    WHERE a2.outlet_id = o.id
) c ON true
